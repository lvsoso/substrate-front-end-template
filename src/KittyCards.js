...
const AskModal = props => {
  ...
  const [formState, setFormState] = React.useState({ addressTo: null, amount: 0 });
  ...
  return <Modal onClose={() => setOpen(false)} onOpen={() => setOpen(true)} open={open}
    ...
    <Modal.Content><Form>
      <Form.Input fluid label='毛孩 ID' readOnly value={kitty.id}/>
      <Form.Input fluid label='价格' placeholder='价格' type='number' state='amount' onChange={onChange}/>
    </Form></Modal.Content>
    <Modal.Actions>
      <Button basic color='grey' onClick={() => setOpen(false)}>取消</Button>
      <TxButton
        accountPair={accountPair} label='确认设置' type='SIGNED-TX' setStatus={setStatus}
        onClick={confirmAndClose}
        attrs={{
          palletRpc: 'kittiesModule',
          callable: 'ask',
          inputParams: [kitty.id, amount],
          paramFields: [true, true]
        }}
      />
    </Modal.Actions>
...
const KittyCard = props => {
  ...
  return <Card>
    ...
    {/* <Card.Content extra style={{ textAlign: 'center' }}>{ owner === accountPair.address
      ? <AskModal kitty={kitty} accountPair={accountPair} setStatus={setStatus}/>
      : ''
    }</Card.Content> */}
